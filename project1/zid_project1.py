    ...
    with open(pth, 'r') as file:
        for line in file:
            ...
            if len(parts) == 2:
                ...
                if exchange.isalpha() and not(" " in exchange) and ticker.isalpha() and not (" " in ticker):
...
def read_dat(tic):
    ...
    location = os.path.join(DATDIR, f"{tic}_prc.dat")
    lines = []
...
def line_to_dict(line):
    ...
    for i in range(len(COLUMNS)):
        name = COLUMNS[i]
        location = COLWIDTHS[name]
        data = line[start: start + location]
        result[name] = data
    ...
    if tickers_lst is not None:
        ...
        for i in tickers_lst:
            if i not in tic_exchange_dic:
                raise Exception(f"The following tickers are not in tic_exchange_dic: {i}")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        ...
        for i in col_lst:
            if i not in COLUMNS:
                raise Exception(f"The following column names are not in COLUMNS: {i}")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        data = []
        alldata = read_dat(ticker)
        for line in alldata:
            line_dic = line_to_dict(line)
            for name in col_lst:
                specified = {name: line_dic[name]}
                data.append(specified)
        result[ticker] = {'exchange': exchange, 'data': data}
    return result
